...
class SwinUNETR(nn.Module):
        ...
        for iclass in range(self.class_num):
            self.controllers.append(
                nn.Sequential(
                    ...
                    nn.Conv3d(
                        512,
                        sum(self.weight_nums + self.bias_nums),
                        kernel_size=1,
                        stride=1,
                        padding=0,
                    ),
    ...
    def forward(self, x_in, return_feature=False, return_for_distill: bool = False):
        ...
        if self.encoding == "rand_embedding":
            task_encoding = (
                self.organ_embedding.weight.unsqueeze(2).unsqueeze(2).unsqueeze(2)
            )
        elif self.encoding == "word_embedding":
            task_encoding = self.organ_embedding
            task_encoding = task_encoding.unsqueeze(2).unsqueeze(2).unsqueeze(2)
        # task_encoding torch.Size([38, 256, 1, 1, 1])
        ...
        for icls in range(self.class_num):
            x_cond = torch.cat(
                [x_feat, task_encoding[icls].unsqueeze(0).expand(b, -1, -1, -1, -1)],
                dim=1,
            )
